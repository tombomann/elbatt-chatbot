...
class VartaService:
    ...
    async def search_batteries(self, query: str) -> List[Dict]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(self.search_url, params={'q': query}) as response:
        ...
        for selector in product_selectors:
            ...
            if products:
                ...
                for product in products:
                    try:
                        battery = {
                            "name": self._safe_extract_text(product, ['.product-name', '.title', 'h1', 'h2', 'h3'], 'Unknown Battery'),
                            "part_number": self._safe_extract_text(product, ['.part-number', '.sku', '.model-number'], ''),
                            "capacity": self._safe_extract_text(product, ['.capacity', '.ah', '.ampere-hour'], ''),
                            "voltage": self._safe_extract_text(product, ['.voltage', '.volt'], ''),
                            "type": self._safe_extract_text(product, ['.battery-type', '.type'], ''),
                            "price": self._safe_extract_text(product, ['.price', '.cost'], ''),
    ...
    def _parse_battery_details(self, html: str) -> Dict:
        ...
        return {
            "name": self._safe_extract_text(soup, ['.product-title', '.title', 'h1'], 'Unknown Battery'),
            "part_number": self._safe_extract_text(soup, ['.part-number', '.sku'], ''),
            "description": self._safe_extract_text(soup, ['.description', '.product-description'], ''),
            ...
            "availability": self._safe_extract_text(soup, ['.availability', '.stock'], '')
        }
