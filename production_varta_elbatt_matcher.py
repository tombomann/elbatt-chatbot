...
class ProductionVartaElbattMatcher:
    ...
    async def _fetch_elbatt_products(self):
        ...
        try:
            ...
            response = requests.get(
                "https://elbatt.no/twinxml/google_shopping.php", timeout=10
            )
            ...
            for varta_battery in varta_batteries:
                ...
                if match:
                    matched_products.append(
                        {"varta_battery": varta_battery, "elbatt_product": match}
                    )
                else:
                    unmatched_batteries.append(varta_battery)

            logger.info(
                f"Matched {len(matched_products)} batteries, {len(unmatched_batteries)} unmatched"
            )
    ...
    def _extract_vehicle_make(self, vehicle_info: Dict) -> str:
        ...
        try:
            if vehicle_info.get("kjoretoydataListe"):
                ...
                make = vehicle["godkjenning"]["tekniskGodkjenning"]["tekniskeData"][
                    "generelt"
                ]["merke"][0]["merke"]
                return make
        except:
    ...
    def _extract_vehicle_model(self, vehicle_info: Dict) -> str:
        ...
        try:
            if vehicle_info.get("kjoretoydataListe"):
                vehicle = vehicle_info["kjoretoydataListe"][0]
                model = vehicle["godkjenning"]["tekniskGodkjenning"]["tekniskeData"][
                    "generelt"
                ]["handelsbetegnelse"][0]
                return model
        except:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    self.search_url, params={"q": search_query}, headers=headers
                ) as response:
    ...
    def _extract_battery_info(self, item) -> Dict:
        ...
        try:
            ...
            part_elem = item.select_one(
                '.part-number, .sku, .model-number, [class*="part"], [class*="sku"]'
            )
            ...
            price = (
                price_elem.get_text(strip=True) if price_elem else "Pris på forespørsel"
            )
