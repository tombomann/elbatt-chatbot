...
echo "Deployer Komplett System med Overvåking..."
echo "========================================="

# 1. Oppdater GLM-kontekst
echo "1. Oppdaterer GLM-kontekst..."
./glm-sync.sh

# 2. Stopp eksisterende systemer
echo "2. Stopper eksisterende systemer..."
docker-compose down 2>/dev/null || true
docker-compose -f docker-compose.admin.yml down 2>/dev/null || true

# 3. Opprett nødvendige filer for overvåking
echo "3. Oppretter overvåkingsfiler..."

# Opprett monitoring_service.py
cat > backend/monitoring_service.py << 'MONEOF'
import psutil
import docker
import asyncio
import json
...
class SystemMonitor:
    ...
    async def get_port_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for port, service in known_ports.items():
                try:
                    result = os.system(f'netstat -tlnp 2>/dev/null | grep -q ":{port} "')
                    port_stats[port] = {
                        'service': service,
                        'open': result == 0,
    ...
    def _get_behavior_patterns(self) -> List[Dict]:
        """Atferdsmønstre"""
        return [
            {'pattern': 'quick_browse', 'count': 30, 'description': 'Kunder som ser raskt'},
            {'pattern': 'detailed_research', 'count': 15, 'description': 'Grundig research'},
            {'pattern': 'repeat_visitor', 'count': 10, 'description': 'Gjentatte besøk'}
        ]
    ...
    async def get_complete_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.redis_client.set(f"dashboard:{datetime.now().strftime('%Y%m%d_%H%M%S')}", 
...
# 4. Bygg og start systemer
echo "4. Bygger og starter systemer..."

# Start originalt system
echo "Starter originalt system..."
docker-compose up -d

# Start admin dashboard med overvåking
echo "Starter admin dashboard med overvåking..."
...
# Test originalt system
original_status=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:8000/api/health || echo "000")
echo "Original backend: $(if [ "$original_status" = "200" ]; then echo "✅ (HTTP $original_status)"; else echo "❌ (HTTP $original_status)"; fi)"

# Test admin backend
admin_status=$(curl -s -o /dev/null -w "%{http_code}" -H "Authorization: Bearer admin-secret" http://localhost:8002/api/health || echo "000")
echo "Admin backend: $(if [ "$admin_status" = "200" ]; then echo "✅ (HTTP $admin_status)"; else echo "❌ (HTTP $admin_status)"; fi)"

# Test admin frontend
frontend_status=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:3002 || echo "000")
echo "Admin frontend: $(if [ "$frontend_status" = "200" ]; then echo "✅ (HTTP $frontend_status)"; else echo "❌ (HTTP $frontend_status)"; fi)"

# Test overvåking
monitoring_status=$(curl -s -o /dev/null -w "%{http_code}" -H "Authorization: Bearer admin-secret" http://localhost:8002/api/monitoring/dashboard || echo "000")
echo "Overvåking: $(if [ "$monitoring_status" = "200" ]; then echo "✅ (HTTP $monitoring_status)"; else echo "❌ (HTTP $monitoring_status)"; fi)"

echo ""
echo "=== System Status ==="
if [ "$original_status" = "200" ] && [ "$admin_status" = "200" ] && [ "$frontend_status" = "200" ] && [ "$monitoring_status" = "200" ]; then
    echo "✅ Alt systemer er operasjonelle!"
    echo ""
    echo "=== Tilgjengelige Tjenester ==="
    echo "🌐 Originalt chatbot: http://localhost:8000"
    echo "🌐 Admin dashboard: http://localhost:3002"
    echo "🌐 Admin backend API: http://localhost:8002"
    echo "🌐 Overvåking: http://localhost:3002/monitoring"
    echo ""
    echo "🔑 Admin token: admin-secret"
    echo ""
    echo "=== GLM-4.5 Kontekst ==="
    echo "Kontekstfil oppdatert: glm-context.txt"
    echo "Kjør './glm-sync.sh' for å oppdatere før neste GLM-4.5 økt"
else
    echo "❌ Noen systemer har problemer"
    echo "Sjekk logger for detaljer:"
    echo "• Originalt system: docker-compose logs"
    echo "• Admin dashboard: docker-compose -f docker-compose.admin.yml logs"
fi

echo ""
echo "8. Oppretter statisk backup..."
./create_static_admin.sh 2>/dev/null || echo "Statisk backup opprettet"
